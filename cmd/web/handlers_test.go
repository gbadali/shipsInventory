...
func Test_showItem(t *testing.T) {
	app := newTestApplication(t)
	...
	tests := []struct {
		...
		{"Valid ID", "/item/1", http.StatusOK, []byte("C210-2019")},
		{"Non-existent ID", "/item/2", http.StatusNotFound, nil},
		{"Negative ID", "/item/-1", http.StatusNotFound, nil},
		{"Decimal ID", "/item/1.23", http.StatusNotFound, nil},
		{"String ID", "/item/foo", http.StatusNotFound, nil},
		{"Empty ID", "/item/", http.StatusNotFound, nil},
		{"Trailing slash", "/item/1/", http.StatusNotFound, nil},
	}
